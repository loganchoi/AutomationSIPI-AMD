...
class ClippingForm(Form):
    ...
    def generate_click(self, sender, event):
        ...
        for x in fileList:
            #filename = c:\Users\logachoi\OneDrive - Advanced Micro Devices Inc\Desktop\Python Scripts - AEDT\Final Codebase\109-H64611-00A_02.siw
            curFile = fileName.split("\\")[-1]
            curFile = curFile.replace(".siw","")
            newFilename = r"\{}_pcb_2dimm_ch-{}_typZ.siw".format(curFile,x)
            ...
            # #Convert IO components to discrete devices
            for io in oDoc.ScrGetComponentList("input/output"):
                comp,ref = io.split()
                oDoc.ScrChangePartType(comp,"Discrete Device")
